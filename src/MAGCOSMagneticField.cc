//Implementation of the Earth's and magnetospheric field.
...
////////////////////////////////////////////////////////////////////////////////
//
MAGCOSMagneticField::MAGCOSMagneticField():fMpl(nullptr)
{ 
  //messenger
  ...
  //fEquationOfMotion = new G4MonopoleEquation(this); 
  ...
  ul_ifail = -999999;
#endif  
  G4cout << "magcos magnetic field" << G4endl; 
}
////////////////////////////////////////////////////////////////////////
//
...
{ n_tsy_data=0;
  ...
  std::ifstream File_Input(nameFile);
 //clear vectors
  igrf_year.clear();
  h_nm.clear();
  g_nm.clear();
  ...
  G4cout << "Reading IGRF table" <<G4endl; 
  recurrence_coef.clear();
 
  G4cout << "Name of the file " << nameFile << G4endl;
 ...
 //Process the first line 
  ...
  while (ch != '\n') {
  	astream<<ch;
	//G4cout << "ch is " << ch  << "  "  << astream.str() << G4endl;
   	File_Input.get(ch);
  }
  G4String str1;
  G4cout << astream.str() << G4endl;
  astream>>str1>>str1>>str1;
  //G4cout << "checking if this works" << str1 << G4endl;
  if (str1!="m"){
  ...
  //G4cout << "igrf year " << year << G4endl;
  //number of column
 
  G4int nyear=igrf_year.size();
  //G4cout << "year size"<< nyear << G4endl;
  nmax=0;
  G4int n,m;
  ...
  //G4cout << "while loop starting" << G4endl;  
  //while (!File_Input.eof()){
  while (File_Input){
	str1="bibab";
    	n=-999;
    	m=-999;
	//G4bool isEOF = File_Input.eof();
	//G4cout << "end of file? " << isEOF << "\n\n" << G4endl;
    	File_Input>>str1>>n>>m;
	//astream>>str1>>n>>m;
	//G4cout << str1 << n << m << G4endl;
	//G4cout << "inside the loop 1 " << G4endl;
    	if (str1 != "bibab" && n != -999 && m != -999) {
     		...
     		if (n>nmax){
				...
				//G4cout << "h_nm [i] is :" << h_nm[i].end() << G4endl;
	 			g_nm[i].insert(g_nm[i].end(),nmax+1,0.);
				//G4cout << "g_nm [i] is :" << g_nm[i].end() << G4endl;
			}
  ...
  //G4cout << "if statement ends" << G4endl;
  
  }
 
 //G4cout << "calculation part reached" << G4endl;
 ...
 G4cout << "read IGRF file in G4 "<< G4endl;

 //G4cout << "h_nm array is " << h_nm << G4endl;
 
 //for (float i: h_nm)
    //G4cout << "h_nm vector is " << i << G4endl;
  /*g_nm.clear();
  dh_nm.clear();
  dg_nm.clear();
  hh_nm.clear();
...
//
G4ThreeVector
   MAGCOSMagneticField::GetTSY2004( G4ThreeVector pos) const
{ 
 SpaceCoordinateConvertor* theConvertor =
                             SpaceCoordinateConvertor::getInstance(); 
...
{ if (fStepper) delete fStepper;
  ...
  /*if (fMpl){
	G4cout << "MONOPOLE in ACTION" << G4endl;
  	if (fMonopoleChordFinder) delete fMonopoleChordFinder;
	fMonopoleEquation = new G4MonopoleEquation(this);
	fMonopoleStepper = new G4ClassicalRK4( fMonopoleEquation, 8 );
	G4double min_step_monopole = 0.01*mm;
	fMonopoleChordFinder = new G4ChordFinder( this, min_step_monopole, fMonopoleStepper);
	fMonopoleChordFinder->SetDeltaChord(DeltaChord);
  }*/

  G4cout << "MONOPOLE in ACTION" << G4endl;
  if (fChordFinder) delete fChordFinder;
  //fMonopoleEquation = new G4MonopoleEquation(this);
  fStepper = new G4ClassicalRK4( fEquationOfMotion, 8 );
  //fStepper = new G4ClassicalRK4( fMonopoleEquation, 8 );
  G4double min_step_monopole = 0.000001*re;
  //G4double min_step_monopole = 0.01*mm;
  fChordFinder = new G4ChordFinder( this, min_step_monopole, fStepper);
  fChordFinder->SetDeltaChord(DeltaChord);
